...
def test_vendor_brand_pending_hidden_then_approved(tokens):
    ...
    pub = requests.get(f"{API}/solar/epc/brands", timeout=30).json()
    ids = [b["id"] for b in pub]
    assert bid not in ids

    # Admin approves
    ap = requests.post(f"{API}/solar/epc/brands/{bid}/approve", headers=_h(tokens["admin"]), timeout=30)
    assert ap.status_code == 200, ap.text
    assert ap.json()["status"] == "approved"

    # Public now sees it
    pub2 = requests.get(f"{API}/solar/epc/brands", timeout=30).json()
...
def test_reject_requires_reason(tokens):
    ...
    empty = requests.post(f"{API}/solar/epc/brands/{bid}/reject", json={"reason": ""}, headers=_h(tokens["admin"]), timeout=30)
    assert empty.status_code in (400, 422), empty.text

    ok = requests.post(f"{API}/solar/epc/brands/{bid}/reject", json={"reason": "not standard"}, headers=_h(tokens["admin"]), timeout=30)
    assert ok.status_code == 200, ok.text
    assert ok.json()["status"] == "rejected"
    # verify persisted rejection_reason via manage endpoint
    mng = requests.get(f"{API}/solar/epc/brands/manage", headers=_h(tokens["admin"]), timeout=30).json()
...
def test_vendor_cannot_approve(tokens):
    ...
    r = requests.post(f"{API}/solar/epc/brands", json=payload, headers=_h(tokens["vendor"]), timeout=30)
    bid = r.json()["id"]
    v = requests.post(f"{API}/solar/epc/brands/{bid}/approve", headers=_h(tokens["vendor"]), timeout=30)
...
def test_admin_package_auto_approved_public_visible(tokens):
    ...
    r = requests.post(f"{API}/solar/epc/packages", json=pkg, headers=_h(tokens["admin"]), timeout=30)
    assert r.status_code in (200, 201), r.text
    pid = r.json()["id"]
    assert r.json()["status"] == "approved"
    pub = requests.get(f"{API}/solar/epc/packages", timeout=30).json()
    assert pid in [p["id"] for p in pub]
    requests.delete(f"{API}/solar/epc/packages/{pid}", headers=_h(tokens["admin"]), timeout=30)
...
def test_vendor_package_pending_hidden_then_approved(tokens):
    ...
    r = requests.post(f"{API}/solar/epc/packages", json=pkg, headers=_h(tokens["vendor"]), timeout=30)
    assert r.status_code in (200, 201), r.text
    pid = r.json()["id"]
    ...
    ap = requests.post(f"{API}/solar/epc/packages/{pid}/approve", headers=_h(tokens["admin"]), timeout=30)
    assert ap.status_code == 200
    pub2 = requests.get(f"{API}/solar/epc/packages", timeout=30).json()
...
def test_freelancer_enquiry_unread_and_mark_read(tokens):
    # find architect
    fls = requests.get(f"{API}/freelancers", timeout=30).json()
    ...
    r = requests.post(f"{API}/freelancers/{arch_id}/enquiry", json=body, headers=_h(tokens["customer"]), timeout=30)
    assert r.status_code in (200, 201), r.text

    # architect fetches
    time.sleep(1)
    e = requests.get(f"{API}/freelancers/me/enquiries", headers=_h(tokens["architect"]), timeout=30)
    ...
    # mark read
    mr = requests.post(f"{API}/freelancers/me/enquiries/mark-read", headers=_h(tokens["architect"]), timeout=30)
    assert mr.status_code == 200
    e2 = requests.get(f"{API}/freelancers/me/enquiries", headers=_h(tokens["architect"]), timeout=30).json()
    assert e2["unread"] == 0

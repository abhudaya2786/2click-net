  ...
  return (
    <div id="voice-assistant-overlay" className="relative z-40">
      <div className="fixed bottom-[calc(var(--app-bottom-h)+0.75rem)] md:bottom-4 right-4 z-40 flex flex-col items-end gap-2 max-w-sm w-full pointer-events-none">
        ...
        <div className="pointer-events-auto flex flex-col items-stretch gap-1.5 p-1.5 pl-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl backdrop-blur-md max-w-[min(100%,22rem)]">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 leading-snug truncate">
                {commandSession.status === 'recording'
                  ...
                  : isListening
                    ...
                    : status === 'permission_needed'
                      ...
                      : status === 'error'
                        ? 'Speech error — Start button use karo'
                        : 'Mic tap / Start / Auto start'}
              </span>
            </div>
          </div>
          ...
          {commandSession.status === 'recording' ? (
            <button
              ...
              className="px-3 py-2 rounded-full bg-rose-600 hover:bg-rose-700 text-white text-xs font-extrabold shadow-sm cursor-pointer shrink-0"
            ...
            <button
              ...
              className="px-3 py-2 rounded-full bg-[#00BAF2] hover:bg-[#0099cc] text-white text-xs font-extrabold shadow-sm cursor-pointer shrink-0"
          ...
          <button
            ...
            className={`p-2.5 rounded-full font-bold text-sm transition cursor-pointer shrink-0 ${
              isListening
          ...
          {onNavigateToSettings && (
            <button
              ...
              className="p-2.5 rounded-full text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer shrink-0"
          ...
          <div className="flex items-center justify-between gap-2 px-1 pb-0.5">
            <button
              type="button"
              onClick={() => {
                const next = !config.autoStartListening;
                updateConfig({ autoStartListening: next });
                if (next) void startListening();
                else stopListening();
              }}
              className={`text-[10px] font-extrabold uppercase tracking-wide px-2.5 py-1 rounded-full border cursor-pointer transition ${
              ...
              title="App khulte hi listening auto start"
            >
              Auto start {config.autoStartListening ? 'ON' : 'OFF'}
            </button>
            <span className="text-[10px] text-slate-400 truncate">
              Settings → Voice → General
            </span>
          </div>
        </div>
      </div>

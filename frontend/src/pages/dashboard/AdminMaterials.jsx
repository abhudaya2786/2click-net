...
export default function AdminMaterials() {
  ...
  const [form, setForm] = useState({ category: "", name: "", brand: "", unit: "unit", rate: "" });
  ...
  const add = async () => {
    ...
    await api.post(`${A}/mart/materials`, { ...form, rate: Number(form.rate) });
    toast.success("Material added"); setForm({ category: "", name: "", brand: "", unit: "unit", rate: "" }); load();
  };
  ...
  return (
    <div className="space-y-5" data-testid="admin-materials">
      <div className="bg-card border border-border p-4 grid md:grid-cols-6 gap-2 items-end">
      ...
      <div className="bg-card border border-border overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-left text-xs uppercase tracking-wider text-muted-foreground">
            <th className="p-3">Category</th><th className="p-3">Material</th><th className="p-3">Brand</th><th className="p-3">Unit</th><th className="p-3">Rate (editable)</th><th className="p-3">Status</th><th className="p-3 text-right">Actions</th></tr></thead>
          <tbody>
            {filtered.map((m) => (

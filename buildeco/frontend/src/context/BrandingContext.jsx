...
function isForeignBrand(data) {
  const name = String(data?.brand_name || "");
  return /2click/i.test(name);
}

function mergeBrand(data) {
  if (!data || isForeignBrand(data)) return DEFAULTS;
  return { ...DEFAULTS, ...data, theme: { ...DEFAULTS.theme, ...(data.theme || {}) } };
}

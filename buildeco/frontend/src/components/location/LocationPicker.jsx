...
export default function LocationPicker({ value = {}, onChange, className = "", pincodeFirst = false }) {
  ...
  useEffect(() => {
    ...
    api.get("/geo/districts", { params: { state: v.state } })
      .then(({ data }) => setDistricts(data.districts || fallbackDistricts(v.state)))
      .catch(() => setDistricts(fallbackDistricts(v.state)));
  }, [v.state]);
  ...
  useEffect(() => {
    ...
    api.get("/geo/cities", { params })
      .then(({ data }) => setCities(data.cities || fallbackCities(v.state, v.district)))
      .catch(() => setCities(fallbackCities(v.state, v.district)));
  }, [v.state, v.district]);
  ...
  useEffect(() => {
    ...
    api.get("/geo/pincodes", { params })
      .then(({ data }) => setPinOptions(data.pincodes || fallbackPincodes({ state: v.state, city: v.city, district: v.district })))
      .catch(() => setPinOptions(fallbackPincodes({ state: v.state, city: v.city, district: v.district })));

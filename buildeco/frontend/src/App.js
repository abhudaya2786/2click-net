...
function AppRouter() {
  ...
  // Hostinger 404.html stores the original path then sends the user to /
  useEffect(() => {
    try {
      const saved = sessionStorage.getItem("buildeco_spa_redirect");
      if (saved && saved !== "/" && saved !== location.pathname + location.search + location.hash) {
        sessionStorage.removeItem("buildeco_spa_redirect");

...
import { fallbackStates, fallbackCities } from "@/lib/geoFallback";
...
export default function EquipmentRental() {
  ...
  useEffect(() => {
    if (state) {
      api.get("/geo/cities", { params: { state } }).then(({ data }) => setCities(data.cities || fallbackCities(state))).catch(() => setCities(fallbackCities(state)));
    } else setCities([]);
  }, [state]);

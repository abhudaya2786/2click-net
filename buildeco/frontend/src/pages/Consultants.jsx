...
import { DEMO_CONSULTANTS, DEMO_CONSULTANT_META } from "@/lib/demoData";
...
function filterDemoConsultants(role, experience, q) {
  return DEMO_CONSULTANTS.filter((c) => {
    if (role !== "all" && c.consultant_role !== role) return false;
    if (experience !== "all" && c.experience_level !== experience) return false;
    if (q) {
      const blob = `${c.name} ${c.title || ""} ${c.bio || ""} ${c.specializations?.join(" ") || ""}`.toLowerCase();
...
export default function Consultants() {
  ...
  useEffect(() => {
    api.get("/consultants/meta")
      ...
      .catch(() => setMeta(DEMO_CONSULTANT_META));
  }, []);
  ...
  const load = useCallback(() => {
    ...
    api.get("/consultants", { params })
      .then(({ data }) => {
        const rows = Array.isArray(data) ? data : data?.consultants || [];
        setList(rows.length ? rows : filterDemoConsultants(role, experience, q));
      })
      .catch(() => setList(filterDemoConsultants(role, experience, q)))

...
export default function Store() {
  ...
  useEffect(() => {
    api.get("/store/meta").then(({ data }) => setMeta(data)).catch(() => {
      const categories = [...new Set(DEMO_STORE_ITEMS.map((i) => i.category).filter(Boolean))];
      const brands = [...new Set(DEMO_STORE_ITEMS.map((i) => i.brand).filter(Boolean))];
      setMeta({ categories, brands, verticals: [] });
    });
  }, []);

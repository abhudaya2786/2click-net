...
- **Frontend (Vercel):** Connect repo to Vercel; uses `vercel.json` (builds `frontend/`, rewrites SPA + `/api`).
- **Frontend (Hostinger):** `cd frontend && npm run build`, then upload `frontend/build/` to `public_html`. Keep the included `.htaccess` so `/store`, `/tenders`, `/login` (refresh / direct links) serve `index.html` instead of Hostinger’s 404 page.
- **Backend:** Deploy `backend/` with `uvicorn server:app` (this repo’s FastAPI app). Set env vars. On Hostinger static hosting, the frontend talks to `https://wallet-vendor-mvp.emergent.host` automatically. Redeploy that host from this `backend/` so `/api/consultants`, `/api/locales`, `/api/geo/*` exist. Until then the UI uses bundled demo data for those options.
